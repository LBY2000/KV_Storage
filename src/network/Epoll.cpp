...
std::vector<Channel*> Epoll::Poll(int timeout){  //epoll的处理模块
...
void Epoll::UpdateChannel(Channel *ch){
    ...
    if(!ch->GetInEpoll()){
        ErrorIf(epoll_ctl(epfd_, EPOLL_CTL_ADD, fd, &ev) ==-1, "epoll add error");//此时红黑树上还没有这个fd，所以需要添加
        ch->SetInEpoll(); //初始化添加完成，下次相同的channel就得是修改了
    } else{
